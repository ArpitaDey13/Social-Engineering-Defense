...
def spam():
    ...
    if request.method == "POST":
      ...
      if b==1:
         prediction="spam"
         return render_template('spam3.html',prediction="{}".format(prediction))
      else:
        prediction= "not spam"
        return render_template('spam3.html',prediction="{}".format(prediction))
    else:
...
def phishing():
    ...
    if request.method == "POST":
      ...
      pred=urlmodel.predict(final_data)
...
def creditcard():
    prediction=""
    if request.method == "POST":
        all_inputs = [request.form.get('Type') , request.form.get('amount') ,
                      request.form.get('oldbalance'),request.form.get('newbalance')]
        ...
        if pred==1:
         prediction=" May be a Fraud transaction"
         return render_template('banking.html',prediction="{}".format(prediction))
        else:
         prediction= "May be not a Fraud transaction" 
         return render_template('banking.html',prediction="{}".format(prediction))
    else:
      return render_template('banking.html',prediction="{}".format(prediction))
